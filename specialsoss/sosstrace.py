...
def trace_fit(order1_poly, order2_poly):
    """
...
def get_counts(params, col):
    ...
    return counts1, counts2, func_counts, true_counts, residual


def trace_polynomials(frame, orders=[4, 4, 4], ref_pix=4, F277W=False, **kwargs):
    """
    Derive the trace polynomials for orders 1 and 2 from scratch. Takes about 5 minutes.
    ...
    stop_2 = 0 if F277W else 1050 if frame.shape[1] == 96 else 1800

    # Stop fitting order 3 at this column
    stop_3 = 0 if (frame.shape[1] == 96 or F277W) else 800
    ...
    if F277W:
        start_params = fit_psfs(frame[start_col], ord12_x0=72, order2=False, order3=False)
        results_right = []

    else:
        start_params = fit_psfs(frame[start_col], ord12_x0=43, ord22_x0=117, order3=False)
    ...
    for idx, col in enumerate(frame[ref_pix:min(stop_1, start_col)][::-1]):
        ...
        if col_num < 600:
            center = np.interp(col_num, [0, 600], [109, 98])
    ...
    start_params = fit_psfs(frame[ref_pix + 1], order1=False, order2=False, ord32_x0=150, plot=False)
    params = start_params
    ...
    image = copy(frame)
    image[image < 1.] = 1.
    mapper = LogColorMapper(palette='Viridis256', low=image.min(), high=image.max())
    fig = figure(width=900, height=200, x_range=(0, 2048), y_range=(0, 256))
    fig.image(image=[image.T], x=0, y=0, dh=256, dw=2048, color_mapper=mapper)

    # Fit polynomial to order 1 centers
    o1_x0 = np.array(ref_cols + [i['ord12_x0'] for i in results] + ref_cols)
    o1_nums = np.argwhere(~np.isnan(o1_x0))
    o1_x0 = o1_x0[o1_nums.flatten()]
    o1_x = np.arange(stop_1)[o1_nums].flatten()
    if len(o1_x) >= 2:
        o1_spline = CubicSpline(o1_x, o1_x0)
        o1_fit = o1_spline(np.arange(stop_1))
        fig.x(o1_x, o1_x0, color='magenta')
        fig.step(np.arange(stop_1), o1_fit, color='cyan')
    else:
        o1_fit = None

    # Fit polynomial to order 2 centers
    o2_x0 = np.array(ref_cols + [i['ord22_x0'] for i in results] + ref_cols)
    o2_nums = np.argwhere(~np.isnan(o2_x0))
    o2_x0 = o2_x0[o2_nums].flatten()
    o2_x = np.arange(2048)[o2_nums].flatten()
    if len(o2_x) >= 2:
        o2_spline = CubicSpline(o2_x, o2_x0)
        o2_fit = o2_spline(np.arange(stop_2))
        fig.x(o2_x, o2_x0, color='magenta')
        fig.step(np.arange(stop_2), o2_fit, color='cyan')
    else:
        o2_fit = None

    # Fit polynomial to order 3 centers
    o3_x0 = np.array(ref_cols + [i['ord32_x0'] for i in results] + ref_cols)
    o3_nums = np.argwhere(~np.isnan(o3_x0))
    o3_x0 = o3_x0[o3_nums].flatten()
    o3_x = np.arange(2048)[o3_nums].flatten()
    if len(o3_x) >= 2:
        o3_spline = CubicSpline(o3_x, o3_x0)
        o3_fit = o3_spline(np.arange(stop_3))
        fig.x(o3_x, o3_x0, color='magenta')
        fig.step(np.arange(stop_3), o3_fit, color='cyan')
    else:
        o3_fit = None
...
def fit_psfs(data, plot=False, order1=True, order2=True, order3=True, sigma=3.1, fwhm=12, **kwargs):
    ...
    if order1:
        ...
        amp12 = kwargs.get('ord12_amp', amp11 * 0.3)
    ...
    if order2:
        ...
        amp22 = kwargs.get('ord22_amp', amp21 * 0.3)
    ...
    if order3:
        ...
        amp32 = kwargs.get('ord32_amp', amp31 * 0.3)
